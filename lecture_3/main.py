...
def add_student_grade() -> None: 
    ...
    Supports entering a single grade or multiple grades at once, with 'done' input stops the operation
    EXAMPLES:
    Input: 80
    ...
    while True:
        ...
        grades_input: list[str] = split(r'[,;\s]+', input('Enter a grade (or \'done\' to finish): ').strip().lower())
...
def get_report() -> None:
    '''
    This function displays a detailed summary of all students' grades and the minimum, maximum, and overall average grade
...
def get_top_performer() -> None:
    '''
    This function calculates the student with the highest average grade among all others and displays a message
    about him.
    '''

    # Create a filtered list of students where each student has an average grade
    graded_students: list[dict] = list(filter(lambda student: student['average'] != None, students))

    # Select a student with the highest average grade. Returns None if students list is empty
    top_performer: dict | None = max(graded_students, default=None, key=lambda student: student['average'])

    # Display an appropriate message
    if top_performer is None:
        print('There is no student with the highest average grade')
    else:
        print(f'The student with the highest average is {top_performer["name"]} with a grade of {top_performer["average"]:.1f}')
